...
import { FaGlobe, FaServer, FaUsers } from 'react-icons/fa6';
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="pl-14">
      <div className="fixed top-0 bottom-0 left-0 w-14 bg-white border-r">
                ...
                <TooltipTrigger asChild>
                  <Link
                    ...
                    className={`block p-2 rounded-md transition-all border border-transparent group relative ${
                      pathname === link.href ? 'bg-slate-700 shadow-sm !border-slate-300' : 'hover:bg-slate-300/60'
                    }`}
                  >
                    <link.icon
                      size={'auto'}
                      className={`${
                        pathname === link.href ? 'fill-white' : 'fill-slate-700 group-hover:stroke-fill-900'
                      }`}
                    />
                  </Link>
                </TooltipTrigger>
                <TooltipContent className="px-2 py-1" side="right" sideOffset={8}>
